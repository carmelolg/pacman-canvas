...
function geronimo() {
  ...
  function Game() {
    ...
    this.loadMapConfig = async () => {
      return new Promise((resolve, reject) => {
        for (let i = 0; i < mapConfig.posY.length; i++) {
          const item = mapConfig.posY[i]
          for (let j = 0; j < item.posX.length; j++) {

            if ((i == 5 && j == 7) || (i == 5 && j == 8) || (i == 5 && j == 9) || (i == 5 && j == 10)) {

            } else if ((i == 1 && j == 1) || (i == 1 && j == 16) || (i == 11 && j == 1) || (i == 11 && j == 16)) {
              const item2 = item.posX[j]
              if (item2.type == "null") {
                item2.type = "powerpill"
              }
            } else {
              const item2 = item.posX[j]
              if (item2.type == "null") {
        ...
        this.map = game.map = Object.assign({}, mapConfig)
        resolve(this.map)
      })
    }
    ...
    this.getPillCount = () => {
      var temp = 0
      for (let i = 0; i < this.map.posY.length; i++) {
        const item = this.map.posY[i]
        for (let j = 0; j < item.posX.length; j++) {
          const item2 = item.posX[j]
          if (item2.type == "pill") {
            temp++
      ...
      //   $.each(item.posX, function (i2, item2) {
      //     console.log(item2);
      //     if (item2.type == "pill") {
      //       temp++;
  ...
  function Ghost(name, gridPosX, gridPosY, image, gridBaseX, gridBaseY) {
    ...
    this.dazzleImg2.src = "img/dazzled2_bicchiere.svg"
